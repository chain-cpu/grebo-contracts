...
test.beforeEach(async (t) => {
  ...
  const ft = await root.devDeploy("./res/ft.wasm", {
  ...
  const nft = await root.devDeploy("./res/nft.wasm", {
    initialBalance: NEAR.parse("100 N").toJSON(),
    method: "new_default_meta",
    args: { owner_id: root },
  });
  ...
  // const defi = await root.devDeploy("../../res/defi.wasm", {
  //   initialBalance: NEAR.parse("100 N").toJSON(),
  //   method: "new",
  ...
  const tokenReceiver = await root.devDeploy("./res/token-receiver.wasm", {
    initialBalance: NEAR.parse("100 N").toJSON(),
    method: "new",
...
test("Simple GREBO transfer", async (t) => {
...
// test("Simple dummy nft transfer", async (test) => {
//   const { root, alice, nft } = test.context.accounts;
